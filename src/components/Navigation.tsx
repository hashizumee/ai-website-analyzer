...
export default function Navigation() {
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-40 w-full backdrop-blur-lg bg-background/80 border-b border-border">
        <div className="flex h-16 items-center px-4 md:px-8">
          <button
            ...
            className="p-2 mr-4 text-muted-foreground hover:text-foreground hover:bg-accent rounded-lg transition-colors"
          >
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 z-50 h-screen w-72 bg-card border-r border-border flex flex-col transition-transform duration-300 ease-in-out shadow-2xl",
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? "bg-teal-500/10 text-teal-600 dark:bg-teal-500 dark:text-[#05070a] shadow-[0_0_20px_rgba(20,184,166,0.3)] font-medium" 
                      : "text-muted-foreground hover:bg-accent hover:text-foreground"
                  )}
                >
                  <div className={cn(
                    "mr-3 shrink-0",
                    isActive ? "text-teal-600 dark:text-[#05070a]" : "text-muted-foreground group-hover:text-teal-500"
                  )}>
                    {item.icon}
                  </div>
                  <div className="flex-1">
                    ...
                    <div className={cn(
                      "text-xs mt-0.5",
                      isActive ? "text-teal-700 dark:text-teal-950/70 font-medium" : "text-muted-foreground"
                    )}>
                      {item.description}
                    </div>
                  </div>
